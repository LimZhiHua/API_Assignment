...
async function register(req, res) {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .send({ message: "Please specify a teacher and list of students" });
    }

    if (!utils.emailCheck(teacher_email)) {
      return res
        .status(400)
        .send({ message: "Please ensure the teacher has a valid email" });
    }
...
async function commonStudents(req, res) {
  try {
    ...
    if (!teacher_emails) {
      return res
        .status(400)
        .send({ message: "Please specify at least one teacher" });
    }
...
async function retrieveForNotifications(req, res) {
  ...
  try {
    const resp = await service.retreiveForNotification(
      teacher_email,
      student_arr
    );
    ...
    if (err.code === 404) {
      ...
      console.error(
        `Error while trying to retrieve notifications`,
        err.message
      );
      res.status(500).send({ message: err });
    }
